...
async def convert_chunk_to_speech(session, api_key, chunk, model, voice="sky", response_format="aac", speed="1.1"):
...
async def process_text_file(api_key, text_file_path, model_choice):
    text_content = read_text_from_file(text_file_path)
    text_chunks = split_text(text_content)
    output_base_path = Path(text_file_path).with_suffix('.m4a')
