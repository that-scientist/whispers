...
async def convert_text_to_speech(client, text, model, voice="alloy", response_format="aac", speed=1.1):
    """
...
async def process_large_text_file(client, text_file_path, model_choice):
    """
    ...
    if len(chunks) == 1:
        ...
        return await process_single_chunk(client, text_file_path, model_choice, text_content)
    else:
        print("🔄 File exceeds limits - processing in batches")
        return await process_multiple_chunks(client, text_file_path, model_choice, chunks)

async def process_single_chunk(client, text_file_path, model_choice, text_content):
    """
    ...
    try:
        response_data = await convert_text_to_speech(client, text_content, model_choice)
...
async def process_multiple_chunks(client, text_file_path, model_choice, chunks):
    """
    ...
    # Rate limit delays - much more generous with new API
    rate_limit_delay = 0.1 if model_choice == "tts-1" else 0.5
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            response_data = await convert_text_to_speech(client, chunk, model_choice)
...
async def main():
    ...
    print("🎤 Modern OpenAI Text-to-Speech Converter")
    print("=" * 50)
    ...
    # Process the file
    success = await process_large_text_file(client, text_file_path, model_choice)
